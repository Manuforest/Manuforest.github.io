...
(() => {
  ...
  function openDB() {return new Promise((resolve,reject)=>{const request=indexedDB.open('manuforest-writing-desk',1);const timeout=setTimeout(()=>reject(Error('Storage timeout')),2500);request.onupgradeneeded=()=>request.result.createObjectStore('drafts');request.onsuccess=()=>{clearTimeout(timeout);if(ready){request.result.close();return;}resolve(request.result);};request.onerror=()=>{clearTimeout(timeout);reject(request.error);};request.onblocked=()=>{clearTimeout(timeout);reject(Error('Storage blocked'));};});}
  ...
  async function addImages(incoming) {
    ...
    for(const raw of incoming){
      ...
      if(files.length>=90 || files.reduce((sum,f)=>sum+f.size,0)+raw.size>100*1024*1024){notice.textContent='附件总量限制为 90 张 / 100 MiB，请分篇保存。';continue;}
  ...
  $('#import-md').addEventListener('change',async e=>{
    ...
    const token=++generation;const currentRevision=revision;
    let imported;try{imported=await file.text();}catch(_){notice.textContent='文件读取失败';return;}
    if(token!==generation)return;
    if(revision!==currentRevision&&!confirm('读取期间草稿已变化，仍要替换吗？'))return;
    textarea.value=imported;slugInput.value=safeName(file.name.replace(/\.(md|markdown)$/i,''))||'my-note';files=[];notice.textContent='已原样导入 Markdown。外部图片不会在基础预览中联网加载；本地图片请重新添加。';changed();
  });
  $('#new-draft').addEventListener('click',()=>{if(!ready)return;if(!confirm('新建会替换当前本机草稿及附件。已经下载备份了吗？'))return;generation++;textarea.value=template();slugInput.value=`${today()}-note`;files=[];notice.textContent='';changed();});
  ...
  function valid(){if(!ready)throw Error('草稿仍在加载');const s=slug();if(!textarea.value.trim())throw Error('还没有正文。');const {header}=headerAndBody(textarea.value);if(!/^date:\s*['"]?\d{4}-\d{2}-\d{2}/m.test(header)&&!/^\d{4}-\d{2}-\d{2}/.test(s))throw Error('请填写 date: YYYY-MM-DD，或使用日期开头的文件夹名。');return s;}
  ...
  $('#import-trigger').addEventListener('click',()=>{if(ready)$('#import-md').click();});
  $('#image-trigger').addEventListener('click',()=>{if(ready)$('#add-images').click();});
  const grid=$('.editor-grid'),tabs=[...document.querySelectorAll('[data-editor-tab]')];
  function view(name){grid.dataset.view=name;tabs.forEach(tab=>{const selected=tab.dataset.editorTab===name;tab.setAttribute('aria-selected',String(selected));tab.tabIndex=selected?0:-1;});}
  grid.classList.add('has-tabs');
  tabs.forEach((tab,index)=>{
    tab.addEventListener('click',()=>view(tab.dataset.editorTab));
    tab.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?tabs.length-1:(index+(event.key==='ArrowRight'?1:-1)+tabs.length)%tabs.length;view(tabs[next].dataset.editorTab);tabs[next].focus();});
  });
  // Initial restoration must finish before inputs can replace the saved draft.
  textarea.disabled=true;slugInput.disabled=true;
  (async()=>{try{db=await openDB();const draft=await readDraft();if(draft){textarea.value=draft.text||'';slugInput.value=draft.slug||'my-note';files=draft.files||[];}else{textarea.value=template();slugInput.value=`${today()}-note`;}status.textContent='草稿保存在此浏览器';savedRevision=revision;}
    catch(_){textarea.value=template();status.textContent='浏览器存储不可用，请及时导出备份';}
    finally{ready=true;textarea.disabled=false;slugInput.disabled=false;document.querySelectorAll('[data-writer-action]').forEach(button=>button.disabled=false);preview();}})();
})();
